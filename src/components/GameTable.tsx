...
import { Search, Filter, ExternalLink, Clock, Gamepad2 } from 'lucide-react';
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-1">
          {game.platforms.map((platform) => (
            <span
              key={platform.name}
              className={`px-2 py-1 text-xs rounded-full ${GameLibraryManager.getPlatformColor(platform.name)}`}
            >
              {platform.name}
            </span>
          ))}
